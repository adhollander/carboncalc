"""Django REST framework api for carbon calculator. """
...
def altregion(oldregion):
    """Adjusts for alternative region codes, assigning these to standard region codes."""
...
def biomasscalcreq(reqparams, dbconn):
    """Calls biomass_calc given parameters listed in reqparams. """
    dbh = 0
    height = 0
    species = ""
    region = ""
...
def biomasscalc(request):
    """API request for calculating biomass, carbon, and CO2 given species, region, dbh, and height. """
...
def biomasstoCO2req(request):
    """API request for converting biomass to CO2 equivalent. """
    if request.method == 'GET':
        try:
...
#def invagecalc(request):
 ...
 #       pass
def invagecalc(request):
    """API request for calculating dbh and height from species, region, and species' age."""
    dbconn = sqlite3.connect(UrbForDB)
    if request.method == 'GET':
        dbconn = sqlite3.connect(UrbForDB)
        try:
...
@parser_classes((JSONParser,))
def agecalc(request):
    """API request for calculating dbh and height from species, region, and age."""
    dbconn = sqlite3.connect(UrbForDB)
    if request.method == 'GET':
        dbconn = sqlite3.connect(UrbForDB)
        try:
...
def biomassdiff(request):
    """API request for calculating growth given species, region, dbh, and height. """
    dbconn = sqlite3.connect(UrbForDB)
    if request.method == 'GET':
        dbconn = sqlite3.connect(UrbForDB)
        try:
...
def avoidedemissions(request):
    """API request for avoided emissions from tree shading. """ 
    if request.method == "GET":
        try:
            species = request.query_params['spec']
            region = altregion(request.query_params['region'])
...
def futuretreegrowth(spec, region, age, nyrs):
    """Returns an array of dicts giving tree biomass etc for nyrs in future given inputted age."""
    dbconn = sqlite3.connect(UrbForDB)
    errorlist = []
    treearr = []
    try:
...
def futuretreegrowthsize(spec, region, dbh, ht, nyrs):
    """Returns an array of dicts giving tree biomass etc for nyrs in future given starting dbh/ht and number of years.."""
    dbconn = sqlite3.connect(UrbForDB)
    try:
...
def futuretree(request):
    """API request for creating table of future tree values given species, region, and age."""
    if request.method == 'GET':
        try:
            species = request.query_params['spec']
            region = altregion(request.query_params['region'])
...
def futuretreesize(request):
    """API request for creating table of future tree values given species, region, dbh, and height."""
    if request.method == 'GET':
        try:
            species = request.query_params['spec']
            region = altregion(request.query_params['region'])
