...
regions <- readRDS("data/regioncombvec.rds") 
speclist <- readRDS("data/speccombvec.rds")
#regionenv <- readRDS("data/regionenv.rds")
#specenv <- readRDS("data/specenv.rds")
regionhash <- readRDS("data/regionhash.rds")
spechash <- readRDS("data/spechash.rds")

# see if parameter a is defined/hashed in environment e, if not just return a              
#envlook <- function(a, e) {
#  if(exists(a, envir=e)) b <- e[[a]]
#  else b <- a
#  b
#}   


hashlooksingle <- function(a, h){
  if(a %in% keys(h))  b<- h[[a]]
  else b <- a
  b
}

# now vectorized
hashlook <- function(a, h) {
  v <- sapply(a, hashlooksingle, h)
  v
}   

biomassq <- function(spec, region, dbh, ht) {
  bdf = data.frame(spec=hashlook(spec, spechash), region=hashlook(region, regionhash), dbh=dbh, ht=ht)
  bjson = toJSON(bdf)
